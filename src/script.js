...
function parseTextbookToQuizzes(raw, opts = {}) {
  ...
  const reQStart = /^\s*(\d{1,4})\s*[\)\.\:\-]\s*(.+)$/;       // 1) 1. 1: 1-
const reChoice = /^\s*([A-D])\s*[\)\.\:\-]\s*(.+)$/i;       // A) A. A: A-
...
let lastWrongKey = '';
let sheepOpen = false;
function bindSheepOkButton() {
  const btn = document.getElementById('btnSheepOk');
  const popup = document.getElementById('sheepPopup');
  if (!btn || !popup) return;

  // tránh bind trùng nếu file bị import/execute 2 lần
